...
export async function POST(req: Request): Promise<Response> {
  if (process.platform !== "win32") {
    return Response.json({ error: "Abrir arquivo só no Windows" }, { status: 501 });
  }
  let account = "";
  let postId = "";
  try {
  ...
  if (!item?.relPath) return Response.json({ error: "Item não encontrado" }, { status: 404 });
  const abs = isAbsolute(item.relPath) ? item.relPath : join(ROOT, item.relPath);
  if (!existsSync(abs)) return Response.json({ error: "Arquivo não existe" }, { status: 400 });
